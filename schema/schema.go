...
func update(ctx context.Context, db *database.DB, logger *logging.Logger, currentVersion string) error {
	...
	if currentVersion != "" {
		cmp, err := compareVersions(currentVersion, expectedVersion)
		if err != nil {
			return err
		}
		if cmp > 0 {
			return fmt.Errorf("database schema version %q is newer than supported version %q", currentVersion, expectedVersion)
		}
	}
